...
class DbManager:
    ...
    def store_comments(self):
        ...
        sql =\
            """
            INSERT INTO `tb_comments_negative` (site_idx, content)  
            VALUES
            (%s, %s)
            """
        ...
        for idx, info in self.comment_infos:
            data_to_insert.append((idx, info))
...
def fetch_negative_comments(comment_start_idx: int = 0):
    ...
    for cur_idx, url in enumerate(urls):
        ...
        cur_site_negative_comments = [(cur_site_idx, comment_info) for comment_info in cur_site_comments]
